...
collection[:response][:results].each do |item|
  if item[:type] == "article"
    item[:views] = 0
  end
end

def read_article(collection)
  num = rand(collection[:response][:results].length)
  collection[:response][:results][num][:views] += 1
  return  puts collection[:response][:results][num]
end

def display_views(collection)
  collection[:response][:results].each do |item|
    puts item[:webTitle]
    puts item[:views]
  end
...
read_article(collection)
puts "-----------"
display_views(collection)
puts "-----------"
read_article(collection)
puts "-----------"
display_views(collection)
